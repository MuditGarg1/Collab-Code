export default function setupSockets(io) {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("join-video-room", ({ roomId, role }) => {
      ...
      if (size > 1) {
        socket.to(`${roomId}-video`).emit("peer-joined", { socketId: socket.id, role });
        
        // If the joiner is the host, also tell THEM that someone is already waiting
        if (role === "host") {
          socket.emit("peer-joined", { socketId: "existing", role: "client" });
        }
      }

      // When the interviewee (client) joins, notify the host to re-fetch room details
      // so the feedback form always has the correct interviewee email
      if (role === "client") {
        socket.to(`${roomId}-video`).emit("interviewee-registered", { socketId: socket.id });
      }
    });
    ...
    socket.on("host-end-room", (roomId) => {
      ...
      // Notify ALL participants (including host) that meeting ended
    ...
    // Interviewee manually leaves — notify the host
    socket.on("client-leave-room", (roomId) => {
      socket.to(`${roomId}-video`).emit("client-left");
      socket.to(`${roomId}-chat`).emit("client-left");
      console.log(`👋 Interviewee left room: ${roomId}`);
    });
